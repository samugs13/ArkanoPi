		...
		if(columnaDisplayActiva == 7 && pin == GPIO_LED_DISPLAY_ROW_7) { // Esta condicion equivale a comprobar cuando se termina de hacer un barrido COMPLETO del display
			...
			if(JUGANDO) {
				...
				if (!primeraVezDisplay) { // Codigo que permite repintar la pantalla SIN SCROLL VERTICAL HACIA ABAJO
					...
					printf("\033[A");
					printf("\033[A\033[A\033[C\033[C\033[C\033[C\033[C\033[C\033[C\033[C\033[C\033[C");
				}

				// Actualizo pantalla
				printf("\n\033[7;35mARKANOPI\n");
				if (p) printf("\033[7;31mEN PAUSA\n"); //si estamos en pausa lo indicamos
					else printf("\033[7;32mEN JUEGO\n");
				printf("\033[7;36mP:%d  V:%d\n", puntos, VIDAS); //panel de puntos y vidas
				for(i=0;i<7;i++) {
					for(j=0;j<8;j++)
						if(pseudoMatriz[i][j]) {
							printf("\033[7;0m");
